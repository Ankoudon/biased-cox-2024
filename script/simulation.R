...
for (h in 1:session) {
                  ...
                  for (j in 1:round_num) {
                    ...
                    if (sum(ndata$test_sample == 1) > 0 & tactic != 0) {
                      
                      test_sample_indices <- ndata$permute == 1
                    ...
                    if (j == 91) {
                      ...
                      if (tactic == 1) {
                        ...
                        g_xdata0 <- graph_from_adjacency_matrix(
                          xdata0, mode = "undirected")
                        
                        friend_list <- ego(g_xdata0, order = 1, nodes = subjects, mode = "all")
                        
                        friend_vec <- lapply(friend_list, function(sublist) {
                          sublist[-1]
                        })
                        
                        friend_vec <- unlist(friend_vec)
                        
                        random_subjects <- setdiff(subjects, friend_vec)
                        
                        ndata <- ndata |>
                          mutate(permute = if_else(id %in% random_subjects, 0, 1))
                        ...
                        babble_list <- ego(
                          g_xdata0, order = 1, nodes = subjects, mode = "all")
                        
                        babble_vec <- unique(unlist(babble_list))
                        
                        ndata <- ndata |>
                          mutate(permute = if_else(id %in% babble_vec, 0, 1))
